      ...
      <div class="flex-1 overflow-y-auto px-6 py-4">
        ...
        <div v-else class="space-y-6">
          ...
          <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
            ...
            <!-- Progress Bar -->
            <div v-if="evaluation && (evaluation.status === 'running' || evaluation.status === 'completed' || evaluation.status === 'paused')" class="mt-4">
              ...
              <div v-if="evaluation && evaluation.status === 'running'" class="mt-3">
                <div class="flex items-center justify-between text-xs mb-1">
                  ...
                  <span class="text-gray-900 dark:text-gray-100 font-medium">
                    {{ activeLLMCalls?.size || 0 }} / {{ maxConcurrentCalls }}
                  </span>
                </div>
                <div class="relative w-full h-3 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                  ...
                  >
                    <div class="absolute inset-0 bg-white/20 animate-pulse" v-if="(activeLLMCalls?.size || 0) > 0"></div>
                  </div>
                  <!-- Percentage text overlay -->
                  <div class="absolute inset-0 flex items-center justify-center">
                    <span class="text-xs font-medium text-gray-700 dark:text-gray-300" v-if="(activeLLMCalls?.size || 0) > 0">
            ...
            <div v-if="activeTab === 'prompt'" class="space-y-6">
              ...
              <div>
                ...
                <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg border border-gray-300 dark:border-gray-600">
                  <pre class="whitespace-pre-wrap text-sm text-gray-900 dark:text-gray-100 font-mono">{{ evaluation?.prompt_text || 'No prompt text available' }}</pre>
                </div>
              </div>
            </div>
          </div>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const activeLLMCalls = ref(new Set())
    const maxConcurrentCalls = ref(5)
    ...
    const loadEvaluationData = async () => {
      ...
      try {
        ...
        if (evaluation.value) {
          ...
          if (evaluation.value.status === 'completed' || evaluation.value.status === 'failed') {
            await Promise.all([
              loadResults(),
              loadStats()
            ])
          } else if (evaluation.value.status === 'paused') {
            await loadResults()
            await loadStats()
          }
          ...
          if (evaluation.value.status === 'running') {
            ...
            activeTab.value = 'prompt' // Show prompt tab for running evaluations
          } else {
            activeTab.value = 'results' // Show results tab for others
          }
        }
      } catch (error) {
    ...
    const loadResults = async () => {
      try {
        const response = await evaluationsStore.fetchEvaluationResults(props.evaluationId)
    ...
    const loadStats = async () => {
      try {
        const response = await evaluationsStore.fetchEvaluationStats(props.evaluationId)
    ...
    const fetchAllFilteredResults = async () => {
      ...
      while (hasMore) {
        try {
          ...
          hasMore = response && response.length === 100
          page++
        } catch (error) {
    ...
    watch(() => evaluationsStore.activeLLMCalls, (newCalls) => {
      activeLLMCalls.value = newCalls
    }, { deep: true })
